...
const TABELA_REAJUSTE = {
  0: {
    M: { reajuste: 10, desconto: 10, acrescimo: 17 },
    F: { reajuste: 8, desconto: 11, acrescimo: 16 },
  },
  1: {
    M: { reajuste: 8, desconto: 5, acrescimo: 15 },
    F: { reajuste: 10, desconto: 7, acrescimo: 14 },
  },
  2: {
    M: { reajuste: 15, desconto: 15, acrescimo: 13 },
    F: { reajuste: 17, desconto: 17, acrescimo: 12 },
  },
};
...
function validarDados(idade, sexo, salario_base, anoContratacao, matricula) {
  ...
  if (
    ...
    salario_base === "" ||
    isNaN(parseFloat(salario_base)) ||
    parseFloat(salario_base) < 0
  ) {
    erros.push("Salário base deve ser um número real válido.");
  }
  if (
    anoContratacao === undefined ||
    anoContratacao === "" ||
    !Number.isInteger(Number(anoContratacao)) ||
    Number(anoContratacao) <= 1960
  ) {
  ...
  if (
    matricula === undefined ||
    matricula === "" ||
    !Number.isInteger(Number(matricula)) ||
    Number(matricula) <= 0
  ) {
...
function paginaInstrucoes() {
  const urlExemplo =
    "/?idade=18&sexo=F&salario_base=1700&anoContratacao=2014&matricula=12345";
  ...
  return `
    ...
    <html lang="pt-BR">
      ...
      <body>
        ...
        <p>Acrescente os parâmetros acima na barra de endereço da página atual para calcular o reajuste.</p>
      </body>
    </html>
  `;
}
...
app.get("/", (req, res) => {
  ...
  try {
    const resultado = calcularNovoSalario(
      idade,
      sexo,
      salario_base,
      anoContratacao,
    ...
    res
      .status(500)
      .send(
        '<h1>Erro ao calcular reajuste</h1><p>Tente novamente.</p><a href="/">Voltar</a>',
      );
  }
});
